...
from modbus_tk import LOGGER, defines
...
from modbus_tk.utils import get_log_buffer, threadsafe_function
...
class Master(object):
    ...
    def execute(
        ...
        if function_code == defines.READ_COILS or function_code == defines.READ_DISCRETE_INPUTS:
            ...
            self._is_swap = True
            is_read_function = True
            pdu = struct.pack(">BHH", function_code, starting_address, quantity_of_x * 2)
            if not data_format:
                data_format = ">" + (quantity_of_x * "i")
            LOGGER.debug(f"expected_length = {expected_length}")
            if expected_length < 0:
                # No length was specified and calculated length can be used:
                # slave + func + bytcodeLen + bytecode x 2 + crc1 + crc2
                expected_length = quantity_of_x * 2 + 5
            ...
            if output_value and data_format:
                ...
                for j in output_value:
                    fmt = "I" if j >= 0 else "i"
                    RES = struct.pack("<" + fmt, j)
                    LOGGER.debug(RES.hex())
                    RES = RES[::-1]
                    LOGGER.debug(RES.hex())
                    RES = RES[2:] + RES[:2]
                    LOGGER.debug(RES.hex())
        ...
        LOGGER.debug(f"resquest = {repr(request)}")
            ...
            if return_code > 0x80:
                ...
                if self._is_swap:
                    for i in range(0, len(data), 4):
                        data[i : i + 4] = data[i + 2 : i + 4] + data[i : i + 2]
                    LOGGER.debug(f"data = {repr(data)}")
                try:
